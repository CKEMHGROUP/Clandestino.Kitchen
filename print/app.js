...
// FIX: Bypasses browser security locks to force a direct print stream
function generatePerfectPDF() {
  const voucherHTML = document.getElementById('voucher-doc').innerHTML;
  
  // Open a clean data staging frame matching your exact canvas bounds
  const printWindow = window.open('', '_blank', 'width=1000,height=324');
  
  printWindow.document.write(`
    <html>
    <head>
      <title>Voucher Export</title>
      <style>
        * { box-sizing: border-box; }
        ...
        @media print {
          @page { size: 13.89in 4.49in landscape; margin: 0 !important; }
